...
frequency <- bind_rows(mutate(tidy_treasure_island_t, title = "Treasure_Island"),
  ...
  filter(!(Treasure_Island=="NA"|Kidnapped=="NA")) #%>%

ggplot(data = frequency, mapping = aes(x = Treasure_Island,
                      y = Kidnapped)) +
...
corr <- frequency %>% select(-word) %>% cor()
corr
...
high_kidnapped_low_treasure_freq <- 
  bind_rows(mutate(tidy_treasure_island_t, title = "Treasure_Island"),
                       mutate(tidy_kidnapped_t, title = "Kidnapped")) %>%
  mutate(word = str_extract(word, "[a-z']+")) %>%
  count(title, word) %>%
  group_by(title) %>%
